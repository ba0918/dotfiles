...
`mise/config.toml` の `[tools]`（`github:ba0918/process-wrap`）で導入する。
...
`codex` は `ai/process-wrap/shim/codex` を経由して起動する。
...
process-wrap 自体は mise のディレクトリを追加しない。
...
境界の検証は process-wrap 側のテスト（`~/develop/process-wrap` で `cargo test`）。
