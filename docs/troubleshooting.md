...
| apply しても `~/.claude/CLAUDE.md` などが symlink のまま残る | 既に apply 済みのマシンでは、実体配布に変えた 4 つ（`~/.claude/CLAUDE.md`、`~/.claude/bash-env.sh`、`~/.claude/statusline.py`、`~/.codex/hooks.json`）が旧方式の symlink として残っている。`mise bootstrap dotfiles apply` がこれを実体で置き換えるか、"refusing to overwrite" で拒むか、適用済みと見なして黙って飛ばすかは保証されていないので、apply の後に `test ! -L ~/.claude/CLAUDE.md && test ! -L ~/.claude/bash-env.sh && test ! -L ~/.claude/statusline.py && test ! -L ~/.codex/hooks.json && echo ok` で実体になったことを確かめる。`ok` が出れば完了。拒まれた場合も、黙って飛ばされて `ok` が出ない場合も、対処は同じで、残っているリンクだけを `rm ~/.claude/CLAUDE.md` のように消して（消えるのはリンクで、repo 側の実体は残る）apply し直す。リンクのままだとプロファイルの `ro` はリンク先の実体に効くだけで、`rw` の `~/.claude` / `~/.codex` 直下にあるリンクの名前は隔離の中から差し替えられる（[docs/dependencies.md](dependencies.md) の process-wrap 節） |
