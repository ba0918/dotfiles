...
| codex の hook のたびに `run-if-present: command not found`（exit 127）が出る | process-wrap のシムは起動したシェルの PATH をそのまま渡すので、そのシェルで `command -v run-if-present` を確認する。無ければ `mise install` で入れ、`config.fish` の `mise activate`（対話シェルは installs ディレクトリ、非対話シェルは `--shims` で shims ディレクトリを PATH に置く）が効いているか確認する |
