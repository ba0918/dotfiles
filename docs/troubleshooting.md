...
| apply しても `~/.claude/bash-env.sh` が残る | 過去の apply が template で配った**実体**なので、`[dotfiles]` の宣言から消しても `mise bootstrap` は回収せず、上の行の `find ... -xtype l` にも出ない（リンク切れ symlink ではなく通常ファイルのため）。apply の後に `rm ~/.claude/bash-env.sh` で手で消す。Claude Code がこのファイルを読むのは `~/.claude/settings.json` の `BASH_ENV` 経由だけで、その settings.json は apply の post-dotfiles hook（`ai/claude/build-settings`）が作り直す。`BASH_ENV` が書かれなくなった時点で残骸は読まれないので実害は無いが、紛らわしいので消しておく |
...
| Claude Code の hook のたびに `run-if-present: command not found`（exit 127）が出る / statusline が空 | `~/.claude/settings.json` の `env.PATH`（build-settings 実行時に固定）で `run-if-present` が見つからない。`claude` を起動したシェルの PATH を変えても効かない。`~/.local/share/mise/shims/run-if-present` が無ければ `mise install`（`mise bootstrap` でも可）で `[tools]` の `github:ba0918/run-if-present` を入れる。`jq -r .env.PATH ~/.claude/settings.json` が process-wrap の shim ディレクトリ（`<dotfiles ルート>/ai/process-wrap/shim`）で始まり、次に `~/.local/share/mise/shims` が来ていなければ（どちらも `$DOTFILES_ROOT` / `$HOME` 展開済みの絶対パス）`ai/claude/build-settings` を実行し直す |
| `codex` が `exec: process-wrap: not found`（exit 127）で落ちる | process-wrap 未導入。シムは最終行で `exec process-wrap` するだけで本体の不在を自分では見ないので、PATH に無ければここで落ちる。`cargo install --path ~/develop/process-wrap` で入れる。急ぐときは `PROCESS_WRAP_SHIM_OFF=1 codex ...` で隔離を素通しできる |
